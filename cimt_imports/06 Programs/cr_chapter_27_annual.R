...
nat.gas.df <- base.df %>% 
  filter(hs_code == 271121,
         ...
         year==2019,
         quantity>0) %>% 
  dplyr::group_by(trading_partner, year) %>% 
  dplyr::summarise(quantity = sum(quantity),
                   value = sum(value)) %>% 
  mutate(trading_partner = ifelse(quantity < 75000, "Other", trading_partner)) %>% 
...
ggsave(file.path(output, str_c("Natural-Gas-2019", ".jpeg", sep="")))
...
oil.df <- base.df %>% 
  ...
  mutate(trading_partner = ifelse(quantity < 200000, "Other", trading_partner),
         trading_partner = iconv(trading_partner, from = 'UTF-8', to = 'ASCII//TRANSLIT')) %>% 
  dplyr::group_by(trading_partner) %>% 
  dplyr::summarise(quantity = sum(as.numeric(quantity)),
                   value = sum(as.numeric(value))) %>% 
...
ggsave(file.path(output, str_c("Oil-2019", ".jpeg", sep="")))
